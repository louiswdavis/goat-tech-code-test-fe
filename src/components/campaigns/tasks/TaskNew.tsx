...
const TaskNew = ({ isOpen, campaignId, onSuccess, onClose }: TaskNewProps) => {
  ...
  const loadForm = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/v1/campaigns/${campaignId}/tasks/new`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/v1/campaigns/${campaignId}/tasks`, {
        method: 'POST',
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md shadow-lg">
        <h2 className="text-2xl font-bold text-blue-300 mb-4">Create New Task</h2>
